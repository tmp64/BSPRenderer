...
{
   // Resolve path
   fs::path fullPath = getFileSystem().findExistingFile(filename, std::nothrow);
   ...
   size_t len = (size_t)appfw::getFileSize(f);
   if (plen)
       *plen = len;
...
char *stb_include_string(char *str, char *inject, char *path_to_includes, char *filename, char error[256])
{
   ...
   for (i=0; i < num; ++i) {
      ...
      // #version is added automatically after file is processed
      #endif
      {
         strcpy(temp, "#line ");
