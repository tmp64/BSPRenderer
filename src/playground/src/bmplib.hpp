...
private:
	...
	void iLoveColorTable(Color* colors = nullptr, int size = 0);
};

void bitMapImage<32>::setPixel(int x, int y, Color c) {
	if (x >= w || y >= h) {
...
void bitMapImage<32>::iLoveColorTable(Color* colors, int size) {
	this->colorTbl = byteArray(4, 0x00FF0000) + byteArray(4, 0xFF000000) + byteArray(4, 0x0000FF00);
	this->bfOffBits = byteArray(4, 66);
	this->bfSize = byteArray(4, 66 + static_cast<int>(ceil(this->w * this->h * (static_cast<double>(32) / 8))) + this->koef);
}

void bitMapImage<24>::setPixel(int x, int y, Color c) {
	if (x >= w || y >= h) {
		throw std::invalid_argument("Can't set this pixel");
	}
	int koef1 = (koef / h) * y;
	pixelBuf.bytes[y * w * 3 + x * 3 + 2 + koef1] = c.red;
	pixelBuf.bytes[y * w * 3 + x * 3 + 1 + koef1] = c.green;
	pixelBuf.bytes[y * w * 3 + x * 3 + koef1] = c.blue;
}
...
void bitMapImage<24>::iLoveColorTable(Color* colors, int size) {
...
void bitMapImage<16>::setPixel(int x, int y, Color c) {
	if (x >= w || y >= h) {
		throw std::invalid_argument("Can't set this pixel");
	}
	int koef1 = (koef / h) * y;
	...
	//TODO: хуйня полная, поправить
	int t1 = c.red << 3;
	int t11 = t1 + (c.green >> 3);
	pixelBuf.bytes[y * w * 2 + x * 2 + 1 + koef1] = t11;
	int tfuck = (c.green >> 3) << 3;
	int t2 = (c.green - tfuck) << 5;
	int t21 = t2 + c.blue;
	pixelBuf.bytes[y * w * 2 + x * 2 + koef1] = t21;
}
...
void bitMapImage<16>::iLoveColorTable(Color* colors, int size) {
	this->colorTbl = byteArray(4, 0b0000011111100000) + byteArray(4, 0b1111100000000000) + byteArray(4, 0b0000000000011111);
	this->bfOffBits = byteArray(4, 66);
...
void bitMapImage<8>::setPixel(int x, int y, int colorIndex) {
	...
	int koef1 = (koef / h) * y;
	pixelBuf.bytes[y * w + x + koef1] = colorIndex;
}
...
void bitMapImage<4>::setPixel(int x, int y, int colorIndex) {
	...
	int koef1 = (koef / h) * y;
	char num = pixelBuf.bytes[y * static_cast<int>(std::ceil(static_cast<double>(w) / 2)) + x / 2 + koef1];
	if (x % 2 == 0) {
		num &= 0b00001111;
	...
	pixelBuf.bytes[static_cast<int>(y * std::ceil(static_cast<double>(w) / 2)) + x / 2 + koef1] = num;
}
...
void bitMapImage<1>::setPixel(int x, int y, int colorIndex) {
	...
	int koef1 = (koef / h) * y;
	char num = pixelBuf.bytes[y * static_cast<int>(std::ceil(static_cast<double>(w) / 8)) + x / 8 + koef1];
	switch (x % 8)
	{
	case 0:
		num &= 0b01111111;
	...
	pixelBuf.bytes[y * static_cast<int>(std::ceil(static_cast<double>(w) / 8)) + x / 8 + koef1] = num;
}
...
template <int C>
void bitMapImage<C>::setPixel(int x, int y, int colorIndex){
	static_assert(C && !C, "Giving any other bit value then 1,4,8,16,24,32 is baaaaad. You shouldn't give any other bit value then 1,4,8,16,24,32 because it's baaaaad, Mkay?");
}

template <int C>
void bitMapImage<C>::saveToFile(const char* filename) {
	static_assert(C && !C, "Giving any other bit value then 1,4,8,16,24,32 is baaaaad. You shouldn't give any other bit value then 1,4,8,16,24,32 because it's baaaaad, Mkay?");
}

template <int C>
void bitMapImage<C>::iLoveColorTable(Color* colors, int size) {
	static_assert(C && !C, "Giving any other bit value then 1,4,8,16,24,32 is baaaaad. You shouldn't give any other bit value then 1,4,8,16,24,32 because it's baaaaad, Mkay?");
}